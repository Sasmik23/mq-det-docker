#!/bin/bash
# Complete MQ-Det Pipeline for HMC Pod
...
MODEL_NAME="MQ-GLIP-TINY-CONNECTORS"
OUTPUT_BASE="OUTPUT"
...
if [ -d "DATASET/$DATASET_NAME" ]; then
    ...
    echo "⚠️  Dataset not found at DATASET/$DATASET_NAME"
...
if [ "$goto_evaluation" != "true" ]; then
    ...
    if [ -f "$OUTPUT_BASE/training.log" ]; then
        echo ""
        echo "📈 Final training metrics:"
        grep -E "accuracy|loss|eta|iter" $OUTPUT_BASE/training.log | tail -20 || echo "No metrics found"
    fi
fi
...
python tools/test_grounding_net.py \
    ...
    2>&1 | tee $OUTPUT_BASE/evaluation.log
...
# Display results
if [ -f "$OUTPUT_BASE/evaluation.log" ]; then
    echo "========================================="
    echo "📊 Evaluation Results"
    echo "========================================="
    echo ""
    grep -E "AP|mAP|Recall|Precision" $OUTPUT_BASE/evaluation.log || \
        tail -50 $OUTPUT_BASE/evaluation.log
fi
...
echo ""
echo "  Logs:"
echo "    - $OUTPUT_BASE/training.log"
echo "    - $OUTPUT_BASE/evaluation.log"
echo ""
echo "  Evaluation Results:"
echo "    - $EVAL_OUTPUT/"
echo ""
